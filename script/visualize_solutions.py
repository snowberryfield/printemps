...
class Visualizer:
    ...
    def plot_contribution(
        ...
        for objective_distribution in objective_distributions:
            for objective in objective_distribution["objectives"]:
                record = [objective_distribution["variable"], objective]
                data.append(record)
    ...
    def write_minimum_spanning_tree_dot(
        ...
        graph = nx.Graph()
        label = (
            "Solution network: The minimum spanning tree of a complete graph where nodes denote solutions. \\n"
...
def main() -> None:
    ...
    parser.add_argument("--title", help="add title to figures.", action="store_true")
