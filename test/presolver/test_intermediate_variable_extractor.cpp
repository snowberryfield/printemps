...
TEST_F(TestIntermediateVariableExtractor, extract_intermediate_variables) {
    ...
    /// Extracting
    {
        printemps::presolver::extract_independent_intermediate_variables(  //
            &model,                                                        //
            false);
        ...
        EXPECT_EQ(printemps::model::VariableSense::Intermediate, z(0).sense());
        EXPECT_FALSE(f.is_enabled());

        EXPECT_EQ(printemps::model::VariableSense::Intermediate, w(0).sense());
        EXPECT_FALSE(g.is_enabled());

        EXPECT_EQ(printemps::model::VariableSense::Intermediate, v(0).sense());
        EXPECT_FALSE(h.is_enabled());
    }

    /// Eliminating (Round 1)
    {
        printemps::presolver::eliminate_independent_intermediate_variables(  //
            &model,                                                          //
            false);
