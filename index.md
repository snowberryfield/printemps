...
|     Date      | Update                                                                                            |
|:-------------:|:--------------------------------------------------------------------------------------------------|
| Oct. 13, 2025 | [PRINTEMPS v2.8.0](https://github.com/snowberryfield/printemps/releases/tag/v2.8.0) was released. |
...
- [`example/bin_packing.cpp`](https://github.com/snowberryfield/printemps/tree/master/example/bin_packing.cpp) solves a bin-packing problem which considers minimization of the number of bins to pack all given items. 
...
|        Ubuntu 22.04       | gcc 11.4.0, 12.3.0 <br> clang 13.0.1, 14.0.0, 15.0.7         |
...
```
where the options `CC` and `CXX` respectively designate the paths of C and C++ compilers, which should be specified according to the user's development environment. The built solver will be generated at `build/application/Release/printemps`.

**Note**: The previous `mps_solver` executable is now deprecated. Users are encouraged to use the new standalone solver described above for MPS files. The old `mps_solver` may be removed in future releases.
