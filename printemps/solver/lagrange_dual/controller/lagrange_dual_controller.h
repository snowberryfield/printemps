...
class LagrangeDualController
   ...
   public:
    ...
    inline bool satisfy_not_applicable_skip_condition(
        ...
        if (this->m_model_ptr->reference().number_of_selection_variables() >
                0 ||
            this->m_model_ptr->reference()
                    .number_of_dependent_binary_variables() > 0 ||
            this->m_model_ptr->reference()
                    .number_of_dependent_integer_variables() > 0) {
