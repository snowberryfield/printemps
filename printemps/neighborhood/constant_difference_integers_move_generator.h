...
/*****************************************************************************/
#ifndef PRINTEMPS_NEIGHBORHOOD_CONSTANT_DIFFERENCE_INTEGERS_MOVE_GENERATOR_H__
#define PRINTEMPS_NEIGHBORHOOD_CONSTANT_DIFFERENCE_INTEGERS_MOVE_GENERATOR_H__
...
class ConstantDifferenceIntegersMoveGenerator
    : public AbstractMoveGenerator<T_Variable, T_Expression> {
   private:
   public:
    /*************************************************************************/
    ConstantDifferenceIntegersMoveGenerator(void) {
        /// nothing to do
    }

    /*************************************************************************/
    virtual ~ConstantDifferenceIntegersMoveGenerator(void) {
        /// nothing to do
    }
    ...
    void setup(const std::vector<model_component::Constraint<
        ...
        std::vector<double> constant_values(BINOMIALS_SIZE);

        for (auto i = 0; i < BINOMIALS_SIZE; i++) {
            auto &move = this->m_moves[2 * i];
            move.sense = MoveSense::ConstantDifferenceIntegers;
