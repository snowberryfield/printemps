...
class DependentIntermediateVariableExtractor {
   ...
   public:
    /*************************************************************************/
    DependentIntermediateVariableExtractor(void) {
        this->initialize();
    }
    ...
    inline int extract(const bool a_IS_ENABLED_PRINT) {
        ...
        auto &intermediate_ptrs =
            m_model_ptr->constraint_type_reference().intermediate_ptrs;

        const int INTERMEDIATES_SIZE = intermediate_ptrs.size();
        ...
        for (auto i = 0; i < INTERMEDIATES_SIZE; i++) {
            ...
            if (has_dependent_intermediate_variable_flags[i]) {
                ...
                const double SIGN =
                    sensitivities.at(intermediate_variable_ptr) > 0 ? -1.0
                                                                    : 1.0;
    ...
    inline constexpr int eliminate(const bool a_IS_ENABLED_PRINT) {
        utility::print_single_line(a_IS_ENABLED_PRINT);
        utility::print_message(
            "Eliminating dependent intermediate variables...",
            a_IS_ENABLED_PRINT);
        ...
        /// Objective
        {
            ...
            for (auto &&variable_ptr :
                 m_model_ptr->variable_reference().intermediate_variable_ptrs) {
                if (sensitivities.find(variable_ptr) != sensitivities.end()) {
                    ...
                    const double SIGN =
                        constraint_sensitivities.at(variable_ptr) > 0 ? -1.0
                                                                      : 1.0;
